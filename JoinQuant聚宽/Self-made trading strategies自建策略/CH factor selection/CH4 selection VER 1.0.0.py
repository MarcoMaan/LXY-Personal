...
def set_backtest():
    ...
    set_slippage(FixedSlippage(0))     #将滑点设置为0  <------- 滑点设置为0？
...
def before_trading_start(context):
    ...
    if g.t%g.powerup==0 and g.t%g.tc != 0:     # ver 1.0.0 指数增强
        # 每4天设置一次调仓
        g.if_adjust = True
        set_slip_fee(context)
...
############################# 增强强势股的表现 ###########################################
    ...
    if g.if_adjust == True:
        ...
        Slist = context.subportfolios[0].long_positions.keys()
        #调用最近5天的收盘价并计算收益
        recent_data = get_price(Slist,getDay(todayStr,-1-g.powerup),getDay(todayStr,-1),'1d')['close'][:]
        recent_data_diff = np.diff(np.log(recent_data), axis = 0)
        
        recent_data_powerup(recent_data_diff,Slist,context)
...
def recent_data_powerup(diff,slist,context):
    #定义指数增强的操作
    stock_up =[]
    stock_down = []
    ...
    for i in range(len(slist)):
        if all(diff[:,i]> 0):
            stock_up.append(slist[i])

    if len(stock_up) != 0:

        for k in range(len(slist)-1):
            if sum(diff[:,k])< 0 and sum(diff[:,k]) < sum(diff[:,k+1]):
                stock_down.append(slist[i])   
        else:
            pass 
        stock_down_min = stock_down[0]
    
        total_value = context.subportfolios[0].total_value
    
        order_value(stock_down_min, total_value/len(slist) * 3/4, style=None, side='short', pindex=0, close_today=False)
        temp_cash = context.subportfolios[0].available_cash
        for stock in stock_up:
            order_value(stock,temp_cash/len(slist) * 1/len(stock_up), style=None, side='long', pindex=0, close_today=False)
        for stock in stock_up:
            order_value(stock,temp_cash/len(slist) * 1/len(stock_up), style=None, side='long', pindex=0, close_today=False)
